...
| `mcr_ui` | MCR-TASK-10 | `ui_check.py`: chama `ui/app.py --smoke` no `:98` com o venv. **Passa desde a MCR-TASK-11**; pula com 77 se faltar venv, `app.py` ou display. Com `WE2002_MCR_CARD` ele também dirige os widgets, grava dois cartões, confere o round-trip deles **e planta os quatro controles negativos que o motor do `controls.py` não alcança** (a conversão de volta do arraste e a exibição de um valor fora do onze — os dois precisam de Qt para serem exercitados). Desde a [CORR-MCR-020](/docs/tasks/port-mcr/CORR-MCR-020.md) ele também abre uma cópia com capitão e cobrador **fora** do domínio medido e exige que a tela os **mostre ou os nomeie num rótulo visível** — tooltip não conta |
