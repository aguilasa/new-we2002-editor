...
| *(sem alvo ainda)* | idem, mais `work/looks-camera/` e `work/looks-walk/`; ~2 min (os dois slots) | `python tools/looks/confront.py --silhouette [SLOT]` — a máscara do painel contra a nossa em **oito passadas** do ciclo por slot (desde a LOOKS-TASK-33; antes, três quadros do arquivo), a passada nomeada pelo `frame_number` na montagem de pose e a nossa desenhada por `scene.walk_scene`; controles: o mesmo quadro duas vezes, quadros diferentes. Testemunha a pose e o corpo, **não** o cabelo | — | LOOKS-TASK-28 |
