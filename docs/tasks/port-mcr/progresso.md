...
| Controles negativos | **todos vermelhos**, com e sem fixture. Quantos são, e de que tipo, é a última linha de `python3 tools/mcr/controls.py` — hoje `controls: 24 of 24 red (23 substitutions, 1 new file)`, e o número sobe a cada task que acrescenta um. O `controls.py --self-check` varre este arquivo e o perfil e recusa total copiado que não bate ([CORR-MCR-021](/docs/tasks/port-mcr/CORR-MCR-021.md)) |
