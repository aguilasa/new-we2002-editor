...
$ python tools/looks/oracle.py --keys "Down x6,Right x41" 2             # HEIG em 210 cm
oracle --keys: 0 difference(s) after 47 press(es), ...
$ python tools/looks/oracle.py --keys "Down,Right x31" 2                # SKIN na ponta
...
- **Três linhas `--keys` da Evidência tinham sido abreviadas à mão** — `"<Right
  ...
  `screen.parse_keys` não tinha sintaxe de repetição: copiadas, elas erravam em
  vez de medir. Foram escritas por extenso na
  [`CORR-LOOKS-075`](/docs/tasks/looks/CORR-LOOKS-075.md) e rodadas de novo, com
  a mesma contagem de teclas e 0 diferença — e voltaram a ser curtas na
  [`CORR-LOOKS-082`](/docs/tasks/looks/CORR-LOOKS-082.md), que deu ao
  `parse_keys` **uma** forma de repetição, `Right x41` (a outra candidata,
  `41*Right`, é recusada), e rodou as três de novo na forma nova: as mesmas 10,
  47 e 32 teclas, 0 diferença. **A divisão entre `Down` e `Right`
