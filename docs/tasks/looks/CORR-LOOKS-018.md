...
- [ ] `Primitive` expõe a profundidade e a semitransparência, e o `self_check()`
      afirma as duas para `0x0018` e para `0x0099`
- [ ] a §1.6 diz quantas primitivas há em cada página e com que profundidade,
      com os números saindo de comando e não de prosa
- [ ] a §1.7 não afirma 4 bpp para tudo, e registra as duas páginas sem entrada
      no `DAT2D.BIN`
- [ ] `python tools/looks/selftest.py` verde, com os 11 controles vermelhos
- [ ] `python tools/looks/modelfile.py --check-image` continua `ok`
- [ ] `roms/` intocada
