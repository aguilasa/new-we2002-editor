...
- [x] A câmera do close-up por linha — **seis das doze aproximam**, medidas no jogo e não escolhidas pelo nome ([LOOKS-TASK-40](/docs/tasks/looks/40-a-camera-do-close-up.md)): `oracle.py --closeups` anda as doze linhas nos dois slots e lê a câmera em cada uma — `SKIN`, `HAIR`, `H.COL`, `FACE` e `H.F.COL.` aproximam na cabeça com a translação `[-120, 266, 999]`, e **`BOOTS`, que não é linha de cabeça, aproxima nas chuteiras** com `[-224, 90, 1934]`; as outras seis ficam com a câmera de corpo inteiro, idêntica número a número à da linha de carga, e o `H` é 1376 nas doze. A janela troca de câmera por linha a partir dos arquivos medidos em `work/looks-camera/`, não de uma lista no código. `confront.py --silhouette-closeups` 0 problema nos dois slots, sem ajustar translação: o pior caso é o `SKIN` do goleiro, 402 de 1922 (21%), com o controle de corpo inteiro 2,5x mais longe; e o `looks_ui` fica vermelho no `BOOTS` se a janela para de aproximar.
