...
`DEFPUSHBUTTON`, e o `CEdDlg` não sobrescreve `OnOK`. No Qt, sem botão default
(todos com `autoDefault=false`), o `Return` não encontra destino e é ignorado.
...
decisões são coerentes entre si; o que falta é escrever a segunda.
...
- [ ] `tools/par/8.10-return-nao-dispara.sh` continua mostrando que **nenhum**
      dos dois dispara ação de edição — é a guarda do `autoDefault=false`
- [ ] Se a escolha for reproduzir: o `DefaultTacticsDialog` **não** muda de
      comportamento (lá o `Return` fecha por outro motivo — CORR-WTE-139)
