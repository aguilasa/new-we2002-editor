...
- [ ] O alvo `test` do `wte/Makefile` roda a tabela de comprimento contra os
      **dois** módulos
- [ ] Alterar `decode()` em só um dos dois arquivos **reprova** o teste de
      identidade, com a mensagem nomeando o arquivo divergente
- [ ] Plantar um comprimento errado no `dump_units.py` reprova a tabela
- [ ] `make -C wte check` verde
- [ ] `python3 wte/tools/dump_units.py --check` e
