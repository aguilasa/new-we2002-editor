...
[WTE-TASK-04](/docs/tasks/concluidos/04-mapa-de-handlers.md). Ela tem sete linhas. Duas
...
O "17" está em `docs/tasks/concluidos/04-mapa-de-handlers.md:32` e em
...
| Onde | Diz | Medido |
|---|---|---|
| `docs/tasks/concluidos/32-preco-do-jogador.md:37` | `formulário ficha_creditos_equipo` na tabela de entrada da tarefa | o dono de `etiqprecioClick` é **`jugador`** — a mesma correção que a linha da §5.1 já faz, no outro arquivo que a repete |
| `docs/tasks/concluidos/30-handlers-auxiliares.md:35` | `BitBtn1Click` (**3×**) | **4×** — a mesma contagem que a tabela de homônimos do próprio `.md` já traz |
| `docs/tasks/concluidos/30-handlers-auxiliares.md:36-38` | `SpeedButton2Click`, `Button2Click`, `Image3Click`, `base_teamClick`, `imagen_urlClick` entre os "repetidos por vários formulários" | **uma vez cada** — o `.md` registra só o sexto da mesma lista, `botonClick` |
...
1. Trocar a atribuição da primeira linha: de `§1.4 do plano e WTE-TASK-04` para
   ...
   está escrito.
2. Acrescentar a linha de `docs/tasks/concluidos/32-preco-do-jogador.md`, ao lado da que
...
**Relação com a [CORR-WTE-006](/docs/tasks/concluidos/CORR-WTE-006.md):** a 006 conserta os
...
[CORR-WTE-006](/docs/tasks/concluidos/CORR-WTE-006.md) rodou antes e consertou os seis
...
- `docs/tasks/concluidos/04-mapa-de-handlers.md` — a mesma má-atribuição no Log
