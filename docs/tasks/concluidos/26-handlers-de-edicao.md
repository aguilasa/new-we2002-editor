...
[WTE-TASK-29](/docs/tasks/concluidos/29-camisa-e-bandeira-2d.md), e
`casilla_precioKeyPress` com `etiqprecioClick` são da
[WTE-TASK-32](/docs/tasks/concluidos/32-preco-do-jogador.md). Os 28 restantes são
...
grava é o grupo da [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md), que
...
[CORR-WTE-044](/docs/tasks/concluidos/CORR-WTE-044.md) desfez para a
[WTE-TASK-22](/docs/tasks/concluidos/22-harness-golden.md) e que a decisão de 2026-08-11
desfez para o critério de tela da
[WTE-TASK-25](/docs/tasks/concluidos/25-handlers-de-carga.md). Como as outras duas, **não
...
| **byte** — `golden_check.sh` | que a edição chega ao byte certo da imagem | **[WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md)** |
...
[CORR-WTE-070](/docs/tasks/concluidos/CORR-WTE-070.md).
...
- [x] ~~Golden verde para cada edição, uma por rodada~~ ~~conferência de tela
      ...
      [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md), porque só ela tem
...
disco, e gravar é da [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md). O
...
condicional, que é da [WTE-TASK-32](/docs/tasks/concluidos/32-preco-do-jogador.md).
...
2. **`iguala_nombres` pode não fechar.** Três hipóteses já caíram. A próxima
   ...
   [WTE-TASK-35](/docs/tasks/concluidos/35-divergencias-deliberadas.md), com dono escrito.
...
- **O achado que desbloqueia o resto da task: o teclado chega ao port.**
  ...
  [WTE-TASK-13](/docs/tasks/concluidos/13-trace-de-eventos.md) de que o GTK2 sem
...
- **O que falta para esta task fechar:**
  ...
  - **a régua de byte**, que depende da
    [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md) — ver a seção
...
- **Resumo do que foi feito:**
  ...
  **As duas metades importam, e por razões diferentes.** As quatro não editadas
  ...
  [WTE-TASK-20](/docs/tasks/concluidos/20-round-trip-headless.md) ensinou a exigir. Do
...
- **Arquivos criados/modificados:**
  ...
  - `docs/tasks/concluidos/26-handlers-de-edicao.md`, `docs/tasks/concluidos/27-handlers-de-gravacao.md`
...
- **Resumo do que foi feito:**
  ...
  **O off-by-one dos 23 `dorsalN` acabou.** A
  [CORR-WTE-057](/docs/tasks/concluidos/CORR-WTE-057.md) tinha medido, na tela, que o port
...
- **O modelo do lote, medido (specs ainda por escrever):**
  ...
  - **E a gravação do número não é desta task:** quem leva o valor de volta é o
    ...
    [WTE-TASK-30](/docs/tasks/concluidos/30-handlers-auxiliares.md). Mais um par que
...
- **Resumo do que foi feito:**
  ...
  **E essa é a descoberta que muda a classificação do handler.** `0x00404048` é
  ...
  [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md). Escrevê-la aqui
...
- **O que já foi lido do lote, e vale para quem continuar:**
  ...
  3. **O contador de slots livres de ML é atualizado aqui**, de
     ...
     a [WTE-TASK-33](/docs/tasks/concluidos/33-slots-de-master-league.md) aparecendo por
...
- **E o passo 44 fechou a pergunta em aberto do `casilla_dorsalKeyPress`.** A
  ...
  Sai daqui um número para a [WTE-TASK-32](/docs/tasks/concluidos/32-preco-do-jogador.md):
...
- **E a regra de validação do lote de mover apareceu inteira, com as mensagens
  ...
  [WTE-TASK-33](/docs/tasks/concluidos/33-slots-de-master-league.md) aparecendo aqui pela
  segunda vez.
...
- **O achado que vale para outra task: os dois oráculos concordam na REGRA do
  ...
  Encaminhado para a [WTE-TASK-32](/docs/tasks/concluidos/32-preco-do-jogador.md), escrito
...
- **O que ficou de fora, com dono escrito:** a metade de gravação da
  ...
  veredito `aberto` e a [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md)
...
- **Uma lacuna declarada, e ela aparece na tela:** o `casilla_xmlibres` mostra
  ...
  [WTE-TASK-33](/docs/tasks/concluidos/33-slots-de-master-league.md). A linha do handler
...
- **O achado que quase virou um preço: `* 10000` é a escala do `Currency`.**
  ...
  ([WTE-TASK-32](/docs/tasks/concluidos/32-preco-do-jogador.md)) a leitura "é o preço da
...
- **Duas chaves vizinhas em `.data`, e elas não são a mesma:**
  ...
  [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md); aqui ela só é ligada e
...
- **Um buraco de escopo, sem dono:** as três rotinas de bitmap carregam
  ...
  [WTE-TASK-29](/docs/tasks/concluidos/29-camisa-e-bandeira-2d.md) cobre `uniformes2d` e
...
- **Arquivos criados/modificados:**
  ...
  - `docs/tasks/concluidos/26-handlers-de-edicao.md` — o custo real da passagem 11, a
...
- **Arquivos criados/modificados:**
  ...
  - `docs/tasks/concluidos/26-handlers-de-edicao.md` — este log
...
- **Arquivos criados/modificados:**
  ...
  - `docs/tasks/concluidos/26-handlers-de-edicao.md` — este log e a correção da passagem 13
...
- **As duas conferências prometidas vieram antes do Pascal, e as duas
  ...
  **1. Eles não gravam.** O inventário de chamadas dos 300 bytes de cada um
  ...
  [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md), e por isso os dois
...
- **Veredito `aberto`, e não pela régua de bytes.** É a **metade dos bitmaps**:
  ...
  a [WTE-TASK-29](/docs/tasks/concluidos/29-camisa-e-bandeira-2d.md) cobre uniforme e
...
- **Arquivos criados/modificados:**
  ...
  - `wte/tools/README.md`
  - `docs/tasks/concluidos/26-handlers-de-edicao.md` — três critérios marcados
...
- **Arquivos criados/modificados:**
  - `docs/tasks/concluidos/26-handlers-de-edicao.md` — o critério reescrito, a seção da
...
- **Arquivos criados/modificados:**
  ...
  - `wte/tools/README.md`
  - `docs/tasks/concluidos/progresso.md`, e o frontmatter desta task
...
- **O que esta task NÃO fechou, e tem dono escrito:**
  - **nove handlers em `aberto` pela opção A** — os 7 de mover, o `dorsalClick`
    ...
    [WTE-TASK-27](/docs/tasks/concluidos/27-handlers-de-gravacao.md). Era o previsto desde
    2026-08-12;
  - ~~o `iguala_nombres` que **não acinzenta** no port — defeito medido pela
    ...
    2026-08-18** pela [CORR-WTE-060](/docs/tasks/concluidos/CORR-WTE-060.md): não era
  ...
  - ~~o destino de `[0x00433a10]` — **não medido**, e nomeado como tal no
    ...
    [CORR-WTE-061](/docs/tasks/concluidos/CORR-WTE-061.md): é a largura, por time, do
    ...
    campo, que a [CORR-WTE-064](/docs/tasks/concluidos/CORR-WTE-064.md) fechou no mesmo
  ...
  - ~~o vetor bola→zona e as tabelas da animação, que o
    ...
    pela [CORR-WTE-062](/docs/tasks/concluidos/CORR-WTE-062.md), com as duas auxiliares.
  ...
  - ~~as três carregadoras de bitmap da ficha (`0x00406fe0`, `0x00407110`,
    ...
    2026-08-18** pela [CORR-WTE-063](/docs/tasks/concluidos/CORR-WTE-063.md): não serão
    ...
    [WTE-TASK-35](/docs/tasks/concluidos/35-divergencias-deliberadas.md). A saturação em
