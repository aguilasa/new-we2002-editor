...
|---|---|---|---|
| **Darkensses** | [`we3d`](https://github.com/Darkensses/we3d) — a browser viewer/editor of `.TMD` and `MODEL.BIN` built on Three.js | **MIT** — reusable **with credit**, which this row is | The `MODEL.BIN` section format (`uint32 numVertex`, `uint32 numPrimitive`, 24-byte primitives, 8-byte vertices), the count of 106 sections, and the *method* of deriving the RAM base address without an emulator. Measured independently against our own images in [`docs/ANALISE-REPOS-WE3D-DBMANAGER.md`](docs/ANALISE-REPOS-WE3D-DBMANAGER.md) §2 before use. Its `TMDParser.v2.js` is a cross-check on our counts, **not** a source to transcribe — the stack is web and ours is Python. **Three of its readings were not taken, because they measured wrong or were never needed** (checked 2026-09-17): its reading of the primitive's fields, which the game contradicted live (plan §1.6); its untangled corner order, which the game's display list contradicted (plan §5.3); and its grouping of the 106 sections into 14 players, which nothing here uses and nobody here confirmed (plan §1.5). |
...
| **The Superpack v6 compilers** (many hands, the Spanish/Portuguese/Italian modding scene) | a 31,790-file, 4,830,420,054-byte collection of binaries, sources, tutorials and art, spanning thirteen folders of the ISS/PES/WE line (measured 2026-09-14 by `tools/looks/superpack_count.py`; its `We2002\` subfolder alone is 28,720 files) | **None**, and no single holder to ask | **Nothing is redistributed.** Three things inside it are *read* where they lie: the 50 face renders of `We2002\MCR\We DB - polipoli\Faces\`, used as an independent render corpus (§5.4 of the plan); the scene's own documentation — the CARP offset tables, the `Offsets We2002.txt`, zeta's hair tutorial — used as opinion to be checked; and the tutorial's `cabellowe2002.bmp`, compared byte by byte against the disc by `atlas.py --compare` and found to be a different image than its name says (§1.8). |
