...
| `test_bmp.pas` | WTE-TASK-29, escrito à mão | prova o `we2002_bmp` — o recipiente de 8 bpp em que a paleta mora. Três grupos: a **recusa** (assinatura, 24 bpp, `bfOffBits` deslocado, compressão, cabeçalho do OS/2, arquivo curto — cada um é um jeito de o `.bmp` do usuário quebrar o `0x36` que o original assume sem conferir), a troca de paleta (ordem `B, G, R`, o byte reservado que **não** se escreve, e as duas pontas da assimetria: a bandeira começa na palavra 0 e faz 16, o uniforme começa na **1** e faz 15) e o pixel (linha de baixo primeiro, alinhamento em quatro bytes). `WTE_TEST_BMP` acrescenta um arquivo real da pasta do Obocaman |
| `roteiros/` | WTE-TASK-13 e 19 | os roteiros de trace de evento e o par 07/08 do travamento |
| `roteiros/golden-14-uniforme*.txt` | WTE-TASK-29 | o gate do `grabar_camisetaClick`, no formato `--artefato`: o handler não escreve na imagem, então comparar só as duas ROMs aprovaria um port inerte. A imagem entra na conferência pelo motivo inverso — é assim que se prova que a extração não vazou para dentro dela |
