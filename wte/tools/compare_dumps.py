...
def gerar() -> str:
    ...
    w()
    w("Produto da [WTE-TASK-20](../../docs/tasks/20-round-trip-headless.md). É")
    ...
    w("([`crash.md`](crash.md), [CORR-WTE-044](../../docs/tasks/CORR-WTE-044.md)).")
    w("É o **`we2002_core`** deste repositório, cujo `Load`/`Save` já é")
    w("byte-idêntico ao `ed.exe` nas duas ROMs. A pergunta que ele responde é")
    w("*o que significam estes bytes*, e é exatamente a desta task.")
    w()
    ...
    w()
    w("- **Os 36 `OFS_*` que faltam.** São exatamente os que o `we2002_core`")
    w("  não tem, então não há lado C++ para eles — nenhum diff Pascal × C++")
    w("  poderia mostrá-los. Continuam com a")
    w("  [WTE-TASK-19](../../docs/tasks/19-os-50-offsets-restantes.md).")
    w("- **Comportamento.** Isto é a camada de dados, não os 96 handlers. O")
    w("  gate deles é a WTE-TASK-22, e ele depende de um oráculo que hoje não")
    w("  sobe.")
