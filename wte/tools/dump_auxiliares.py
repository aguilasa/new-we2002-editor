...
def gera_md(m: dict) -> str:
    ...
    a("desconhecido devolve espaço. A conferência aborta se as tabelas")
    a("deixarem de ser identidade, porque nesse dia a palavra \"filtro\" fica")
    a("errada.")
    a("")
    a("**E ela não é só de tela, ao contrário do que este arquivo dizia até")
    a("2026-08-21.** O `jugador.BitBtn3Click` — o `Comple.` da ficha — grava os")
    a("dez bytes de nome a partir do CAMPO da ficha, e o campo mostra o texto")
    a("já filtrado. Com a ROM japonesa isso são quatro `?` gravados sobre o")
    a("nome; o port reproduz o filtro desde a CORR-WTE-081, e é o gate")
    a("`golden-15-ficha` que mede os quatro bytes.")
    a("")
    if com_papel:
