...
| `compara_tela.{sh,py}` ✅ | 25, 26, 29 | a régua de **pixel**: o `.sh` leva os dois lados ao mesmo estado e o `.py` mede. Seis modos — sem opção compara as cinco barras e a montagem de um time; `--habilitacao` compara que controles acinzentam; `--edicao` e `--nomes` editam antes de medir; `--cor` mede as 16 amostras do editor de cor **como vieram da imagem**; e `--grade` mede depois de **calcular** a cor — três cliques em escurecer, um em clarear e um no gradiente —, com a guarda de que a cor mudou nos dois lados, porque comparar duas telas paradas seria concordância em não fazer nada. O `.py` tem `--autoteste`; o `.sh` não é gerador e fica fora do `--check` porque precisa do Wine e do `:98` |
