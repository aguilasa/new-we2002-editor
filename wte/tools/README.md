...
| `dump_mcr.py` ✅ | 28 | o `.mcr`: lê o **contêiner** do molde (`we-team-editor/data/dat.bin`, primeira metade) pela documentação pública do formato e o **conteúdo** do bloco do WE2002 do `.exe`, e emite `re/mcr.{md,tsv}`. `--medir <cartao.mcr>` confere um cartão de verdade contra o molde e escreve `re/mcr-medido.tsv` — a fixture fica fora do git, a medição entra. **Recusa** se a tabela de cobradores (`0x00423F84`) ou a de deslocamentos de bit (`0x0042360C`) deixar de bater com o layout escrito |
...
| `test_dump_mcr.py` ✅ | o contêiner contra a documentação pública (magia, 16 blocos, cadeia do diretório coerente, o resto livre), os dois guards com VA plantado, e o **achado do bloco 3** medido em vez de escrito: há destino de escrita fora dos blocos que o diretório declara, esse bloco vem zerado no molde, nenhum destino cai no diretório, e capitão e cobradores são justamente os que ficam fora |
