...
| `sem_wine.sh` ✅ | 40 | **não gera nada** — fabrica um ambiente **sem Wine e sem 32 bits** num user+mount namespace (`bwrap`), cobrindo com `tmpfs` vazio o runner do Bottles, o `/var/lib/flatpak`, os dois `work/wineprefix*` e o stack `i386`, e roda o comando lá dentro. **Recusa** por duas cláusulas — ambiente que só parece limpo mede tão pouco quanto não medir: se algum alvo mascarado **não ficar vazio** lá dentro, que é a que trabalha nesta máquina, ou se `wine`/`wine64`/`wineserver`/`winecfg` responderem no `PATH`, que é a que pega uma máquina com o pacote do apt. As duas estão medidas no `test_check_nativo.py` (CORR-WTE-120), inclusive a demonstração de que apagar a segunda **desliga** a conferência com a primeira intacta. É `.sh` de propósito — precisa do `bwrap` e fica fora do `--check` |
