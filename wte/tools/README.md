...
| `test_spec_index.py` ✅ | as onze rotas de recusa do `spec_index.py` sobre specs sintéticas: decompilado colado nas sete formas de nome inventado mais o cast do Ghidra (`(int)*(int *)`, com o caso negativo em prosa), veredito e evidência fora do vocabulário, seção faltando, `nao portado` sem justificativa, `implementado` só com observação de tela, spec órfã, frontmatter discordando do TSV. **É a única coisa que mede essas regras** — `--check` verde sobre 96 `aberto` não exercita nenhuma |
