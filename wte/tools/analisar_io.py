...
def secao_passagem_5(w, todas_sessoes, conhecidos, candidatos) -> None:
    ...
    if uni:
        ...
        w("É a entrada da [WTE-TASK-29]"
          "(../../docs/tasks/concluidos/29-camisa-e-bandeira-2d.md), e é a maior região")
...
def gerar() -> str:
    ...
    w("([CORR-WTE-047](../../docs/tasks/concluidos/CORR-WTE-047.md)). Agora o `cmp.tsv` de")
    ...
    if len(imagens) > 1:
        ...
        w("")
    w("### E a região vazia é pista, não causa")
    w("")
    w("A causa foi medida depois, e está em [`crash.md`](crash.md): a")
    w("violação de acesso cai dentro do `vcl60.bpl`, em")
    w("`Graphics::TFont::SetSize`, com o `this` **nulo** — chamada por uma")
    w("rotina do `.exe` que procura um controle pelo nome. **A falha é de")
    w("estado de interface, não de leitura da imagem.** O que esta seção mede")
    w("continua valendo como pista da *causa da causa*, e não como a causa.")
    w("")
    ...
    if ultima:
        ...
        w("")
    w("**O pedido, então, deixou de ser \"a release de 474.431.328 bytes\":**")
    w("truncar a que temos não serve.")
    w("")
    w("**E deixou de ser pedido.** A CORR-WTE-044 mediu a causa e ela não é")
    w("esta: o ponteiro que a rotina de realce usa é sobrescrito pela carga do")
    w("time, o controle procurado **existe**, e a mesma faixa em `14368636` é")
    w("lida pelas **duas** imagens do repositório — só uma trava. Com")
    w("`roms/japanese-shift-jis.bin` o `wte.exe` passa da troca de time com")
    w("zero violação de acesso. Ver [`crash-causa.md`](crash-causa.md).")
    w("")
    ...
    w("")
    w("**A coluna `Bloqueado` vale para a imagem que esta medição usou, a")
    w("europeia.** O `wte.exe` é o **oráculo comportamental** do projeto (§4.2")
    w("do plano), e a")
    w("[WTE-TASK-22](../../docs/tasks/concluidos/22-harness-golden.md) monta o gate golden")
