...
def gerar() -> str:
    ...
    w("objeto, não o byte. A região vazia em `14368636`, que o")
    w("[`offsets-novos.md`](offsets-novos.md) mede, continua sendo um fato —")
    w("mas deixou de ser a explicação, e passou a ser candidata a *causa da")
    w("causa*: um índice derivado de dado ausente pode muito bem ser o que")
    w("leva a rotina a procurar um controle que não existe.")
    w()
    w("Isso **não desbloqueia** a WTE-TASK-19: continua sem haver como levar")
    w("o oráculo comportamental além da tela de carga com as ROMs deste")
    w("repositório. O que muda é o pedido e o próximo passo — a pergunta")
    w("deixou de ser *que imagem tem esta região preenchida* e passou a ser")
    w("*por que este controle não existe*, e essa segunda é respondível com o")
    w("ferramental da fase 4 sobre um manipulador que já tem nome, endereço e")
    w("`depends_on` declarado.")
    return "\n".join(L) + "\n"
