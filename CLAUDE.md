...
| `python tools/looks/oracle.py --walk-watch [SLOT]` | quantas cargas de matriz da caminhada nomeiam o par, pelos dois watches: 520 de 520 no `ANIME_BUILD`, 316 de 520 no `ANIME_UNPACK`, que perde a metade espelhada inteira (204 cargas, 17 passadas); ~2 min |
...
| `python tools/looks/confront.py --outside [SLOT]` | a nossa tela contra a do jogo fora do boneco, região por região — no `python` principal, que é quem tem Pillow; o do venv é lançado só para a janela |
