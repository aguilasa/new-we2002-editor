...
SUBS: list[tuple[str, str, str]] = [
    ...
    (r"fil_ctrl\.Seek\(([^,]+),\s*CFile::begin\)", r"fil_ctrl.Seek(\1)", "Seek(begin) -> Seek"),
    (
        r"fil_ctrl\.Seek\(([^,]+),\s*CFile::current\)",
...
def main() -> int:
    ...
    if problems:
        print("\nRESTOU CONSTRUCAO MFC -- corrija SUBS antes de usar:", file=sys.stderr)
        for p in problems:
